...
<%
    ...
    int level = 0;
    String title = "";
%><li class="dropdown mobile"><a class="dropdown-toggle" data-toggle="dropdown" href="#"> Menu <strong class="caret"></strong> </a><%
    int cycle = 0;
    while (!navigationItems.isEmpty()) {
        ...
        if(level >= startingLevel) {
            if (first) {
                first = false;
%><ul class="dropdown-menu"><% //Con classe "afflix" la navbar rimane sempre visibile, ma bisogna dare width assoluta (Responsive)
}
        ...
        for (NavigationItem current : navigationItems) {
            ...
            if(level >= startingLevel) {
                xb.openElement("li");
                if (current.isInPath()) {
                    xb.addAttribute("class", "dropdown-header");
                    nextNavigationItem = current;
                    xb.write(current.getTitle());
                    ...
                    if (nextNavigationItem != null && level < maxLevel) {
                        ...
                        for (NavigationItem curr : navigationItems) {
                            xb.openElement("li");
                            xb.addAttribute("class", "sub");
                            xb.addAttribute("style","text-indent: 15px;");
