...
<s:if test="#buttonsBarBottom">
    ...
    <s:if test="#position >= 0">
        <div style="float: right;">
            <s:if test="#position > 0">
                <s:url var="firstUrl" namespace="/model" action="%{qualifiedName}/TableData">
                    <s:param name="pk"
                ...
                <s:url var="previousUrl" namespace="/model" action="%{qualifiedName}/TableData">
                    <s:param name="pk"
            ...
            <s:if test="#position < #size-1">
                <s:url var="nextUrl" namespace="/model" action="%{qualifiedName}/TableData">
                    <s:param name="pk"
                             value="%{pkHelper.generatePkString(objects.get(#position+1))}" />
                    <s:param name="searchString" value="%{searchString}" />
                </s:url>
                <s:a id="next" href="%{#nextUrl}">next</s:a>
                <s:url var="lastUrl" namespace="/model" action="%{qualifiedName}/TableData">
                    <s:param name="pk"
