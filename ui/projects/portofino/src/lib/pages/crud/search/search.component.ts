...
export const DEFAULT_SEARCH_TEMPLATE = `
<mat-card>
    ...
    <mat-expansion-panel *ngIf="searchProperties.length">
      <mat-expansion-panel-header>
        <mat-panel-title>
          ...
          <span class="portofino-search-title">{{ 'Search' | translate }}</span>
        </mat-panel-title>
      </mat-expansion-panel-header>

      <form (submit)="search()" [formGroup]="form">
        <button type="submit" hidden>Hidden search button</button>
...
export const DEFAULT_SEARCH_STYLE = `
.search-results table.mat-table {
  width: 100%;

  tr.mat-row[mat-row] td.mat-cell {
    a.mat-button {
      margin-left: -16px;
    }
  }
